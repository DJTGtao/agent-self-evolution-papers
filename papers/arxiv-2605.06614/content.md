...
Siru Ouyang$^{1*}$, Jun Yan$^{2†}$, Yanfei Chen$^{2}$, Rujun Han$^{2}$, Zifeng Wang$^{2}$, Bhavana Dalvi Mishra$^{2}$, Rui Meng$^{2}$, Chun-Liang Li$^{2}$, Yizhu Jiao$^{1}$, Kaiwen Zha$^{3}$, Maohao Shen$^{3}$, Vishy Tirumalashetty$^{2}$, George Lee$^{2}$, Jiawei Han$^{1}$, Tomas Pfister$^{2}$ and Chen-Yu Lee$^{2†}$
...
arXiv:2605.06614v1 [cs.AI] 7 May 2026

SkillOS: Learning Skill Curation for Self-Evolving Agents

![img-0.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-0.jpeg)
...
**Skill Repository.** We maintain an external skill repository $S_t$ at time stamp $t$, which consists of

3

SkiIIOS: Learning Skill Curation for Self-Evolving Agents

![img-1.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-1.jpeg)

Figure 2 | SKILLOS training pipeline. Each training step samples a group of related tasks and initializes an empty SKILLREPO. $\pi_S$ is optimized with composite rewards, enabling self-evolution.

$N_t$ reusable skills $S_t = \{s_t^1, s_t^2, \ldots, s_t^{N_t}\}$. Following the widely adopted SKILL.md format (Anthropic, 2025b), each skill is represented as a single Markdown file with two components as shown in Figure 1(b): (i) YAML frontmatter, which specifies the skill name and a natural-language description of when the skill should be used, and (ii) Markdown instructions, which describe the executable knowledge, workflows, constraints, and reusable heuristics captured by the skill.

Agent Executor. Given a task $x_t$, a frozen agent executor $\pi_{\mathcal{L}}$ solves the task conditioning on the current environment observation and relevant skills. Specifically, we retrieve a subset of skills $\tilde{S}_t \subseteq S_t$ using BM25 (Robertson and Zaragoza, 2009) for each task $x_t$, and the executor samples actions following $a \sim \pi_{\mathcal{L}}(\cdot \mid x_t, o_t, \tilde{S}_t)$.

Skill Curator. After the executor completes task $x_t$, the skill curator $\pi_S$ observes the trajectory $\xi_t$, the self-judged correctness of the answers/interactions $\mathbb{1}_{\xi_t}$, and a retrieved subset of related skills $\tilde{S}_t$. It then generates a sequence of structured curation operations $c_t = (u_t^1, \ldots, u_t^{M_t}) \sim \pi_S(\cdot \mid \xi_t, \mathbb{1}_{\xi_t}, \tilde{S}_t)$, where each operation $u_t^m$ is one of { insert_skill, update_skill, delete_skill }. Each operation is implemented as a function call (detailed signature in Figure 8) that manipulates the skill repository $S_t$. Applying these operations transforms the repository from $S_t$ to $S_{t+1}$ as $S_{t+1} = \text{APPLYOPS}(S_t, c_t)$. The updated repository is then used by the executor on subsequent tasks, forming a closed loop between task execution and experience-driven skill evolution.

### 3.2. Learning Skill Curation with RL

We optimize the skill curator $\pi_S$ with RL and keep the agent executor $\pi_{\mathcal{L}}$ frozen. The main challenge is indirect and delayed feedback for curation decisions, which is only revealed through $\pi_{\mathcal{L}}$'s performance on future relevant tasks. We address this by constructing grouped training instances (§ 3.2.1) and designing a composite reward (§ 3.2.2) that combines future task outcomes with intermediate signals on operation validity, skill quality, and the conciseness of skills. An overview of the training process is shown in Figure 2.
...
$$\mathcal{D} = \{G_1, G_2, \dots, G_M\}, \qquad G_m = \{x_{m,1}, x_{m,2}, \dots, x_{m,|G_m|}\},$$
...
We employ Grouped Reward Policy Optimization (GRPO Shao et al. (2024)) for its training stability and sample efficiency. The training loop shown in Algorithm 1 optimizes the skill curator policy $\pi_S$ to maximize a composite reward function over the distribution of generated traces. For a task group $G = (x_1, \dots, x_{|G|})$, the curator produces a sequence of curation decisions $c = (c_1, \dots, c_{|G|})$ as the executor proceeds through the group. Each training step, the reward combines four signals:

$$r = \underbrace{r^{\text{task}}}_{\text{task outcome}} + \lambda_f \underbrace{r^{\text{fc}}}_{\text{function call}} + \lambda_u \underbrace{r^{\text{cnt}}}_{\text{content quality}} + \lambda_c \underbrace{r^{\text{comp}}}_{\text{compression}} \tag{1}$$

**Task outcome reward.** The first task uses an empty SKILLREPO, before any curator update occurs. We thus define the task outcome reward as the average success over the remaining tasks as $r^{\text{task}} = \frac{1}{|G|-1} \sum_{i=2}^{|G|} \mathbb{1}(\xi_i)$, which provides executor-grounded signal on downstream performance achieved by the evolving SKILLREPO from $\pi_S$.
...
**Compression reward.** To discourage verbatim trajectory copying, we reward concise repository updates. Let $S_i$ denote the skill repository after applying $c_i$, and let $\chi_i$ denote the curator input context at position $i$. We define $r^{\text{comp}} = \frac{1}{|G|} \sum_{i=1}^{|G|} \left(1 - \frac{|S_i|}{|\chi_i|}\right)$, where $|S_i|$ and $|\chi_i|$ denote token lengths. This encourages the curator to distill reusable skills rather than store raw trajectories.
...
For each task group $G$, we sample $N$ independent rollouts of the *entire curation sequence* from $\pi_S$. Within each rollout, the executor produces trajectory $\xi_i$ using the skill repository $S^i$ resulting from previous curations $c_{<i}$ till task position $i$ with the same training task group, so different rollouts evolve
...
2:    \( G = (x_1, \ldots, x_{|G|}) \), \( S \leftarrow \emptyset \)    ▷ Sample a task group and initialize SkillRepo
3:    for task index \( i = 1, \ldots, |G| \) do
4:    \( \tilde{S} \leftarrow BM25(x_i, S) \)    ▷ Retrieve relevant skills
5:    \( \xi_i \leftarrow RUNTASK(\tilde{S}, \pi_{\mathcal{L}}, x_i) \)    ▷ Run inference on frozen executor
6:    \( c_i \sim \pi_S(\cdot | \xi_i, \tilde{S}) \)    ▷ Sample a rollout from skill curator
7:    \( S \leftarrow APPLYOPS(S, c_i) \)    ▷ Apply insert/update/delete
8:    end for
9:    \( r \leftarrow CALCULATEREWARD(\xi, c) \)
10:    UPDATE \( \pi_S \)    ▷ Update skill curator using GRPO
11: end for

different repository histories. The GRPO advantage is computed as: \( A^n = r^n - \frac{1}{N} \sum_{n' = 1}^{N} r^{n'} \), where \( r^n \) is the composite reward (Eq. 1) for the \( n \)-th rollout. We optimize \( \pi_S \) with a clipped surrogate objective over all curation steps \( i = 1, \ldots, |G| \):

\[
\mathcal {L} = \mathbb {E} _ {n} [ \min (\rho^ {n} A ^ {n}, \operatorname{clip} (\rho^ {n}, 1 - \epsilon , 1 + \epsilon) A ^ {n}) ] \tag {2}
\]

where \(\rho^n = \pi_S(c^n\mid \chi) / \pi_{\theta_{old}}(c^n\mid \chi)\) is the importance ratio. The advantage \(A^n\) is assigned uniformly to all tokens in \(c^n\), and we discard the KL term in GRPO to encourage policy exploration.
...
Table 1 | Experiment results on ALFWorld benchmark. Success rate (SR ↑) and the number of steps (Steps ↓) are reported on 6 subsets with 3 different frozen executors.

|  Methods | Curator π_{S} | Pick (35) | Look (13) | Clean (27) | Heat (16) | Cool (25) | Pick2 (24) | Avg. SR (140) | Steps  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
|  *Executor π_{L}: Qwen3-8B*  |   |   |   |   |   |   |   |   |   |
|  No Memory | None | 78.1_{1.6} | 46.2_{7.7} | 33.3_{13.4} | 37.5_{10.8} | 29.3_{6.1} | 47.2_{6.4} | 47.9_{1.2} | 21.1  |
|  ReasoningBank | Qwen3-8B | 83.8_{0.0} | 48.7_{7.2} | 49.4_{16.2} | 39.6_{4.4} | 41.3_{8.5} | 54.2_{8.8} | 55.7_{3.1} | 20.1  |
|  MemP | Qwen3-8B | 80.0_{5.7} | 43.6_{4.4} | 24.7_{4.3} | 33.3_{3.6} | 38.7_{6.1} | 48.6_{6.4} | 49.7_{0.7} | 21.0  |
|  Skillos-base | Qwen3-8B | 79.0_{8.7} | 41.0_{4.4} | 45.7_{4.3} | 37.5_{9.5} | 38.7_{4.0} | 55.6_{2.1} | 53.1_{2.5} | 20.4  |
|  Skillos-gemini | Gemini-2.5-Pro | 77.1_{6.0} | 53.8_{6.1} | 37.0_{6.4} | 37.5_{9.5} | 36.0_{3.2} | 50.0_{6.7} | 50.7_{3.6} | 20.8  |
|  Skillos | Qwen3-8B | **85.7**_{3.3} | **56.4**_{7.7} | **54.3**_{8.6} | **43.8**_{9.5} | **46.7**_{2.3} | **62.5**_{6.4} | **61.2**_{4.6} | **18.9**  |
|  *Executor π_{L}: Qwen3-32B*  |   |   |   |   |   |   |   |   |   |
|  No Memory | None | 80.0_{2.9} | 69.2_{0.0} | 45.6_{7.7} | 37.5_{16.5} | 42.7_{6.1} | 43.1_{2.4} | 54.5_{2.5} | 20.3  |
|  ReasoningBank | Qwen3-8B | 86.7_{3.0} | 71.8_{5.4} | 50.6_{6.3} | 45.8_{13.3} | 52.0_{8.9} | 51.4_{5.1} | 61.4_{2.5} | 18.7  |
|  MemP | Qwen3-8B | 80.0_{2.9} | 76.9_{0.0} | 44.4_{7.4} | 37.5_{10.8} | 42.7_{2.3} | 47.2_{6.4} | 55.7_{3.7} | 20.0  |
|  Skillos-base | Qwen3-8B | 82.9_{2.9} | 69.2_{11.8} | 48.1_{2.1} | 50.0_{9.7} | 48.0_{14.4} | 52.8_{11.0} | 59.8_{3.0} | 19.2  |
|  Skillos-gemini | Gemini-2.5-Pro | **97.1**_{3.0} | 76.9_{5.4} | 55.6_{6.0} | 43.8_{11.3} | 40.0_{5.7} | 54.2_{4.9} | 63.6_{4.2} | 18.1  |
|  Skillos | Qwen3-8B | 91.4_{3.1} | **76.9**_{4.4} | **59.3**_{8.6} | **56.3**_{12.5} | **57.3**_{10.1} | **62.5**_{4.2} | **68.6**_{5.7} | **17.3**  |
|  *Executor π_{L}: Gemini-2.5-pro*  |   |   |   |   |   |   |   |   |   |
|  No Memory | None | 90.5_{3.2} | 66.7_{5.1} | 48.1_{10.2} | 39.6_{17.1} | 68_{7.4} | 68.1_{3.8} | 66.4_{2.0} | 17.7  |
|  ReasoningBank | Qwen3-8B | 91.4_{3.4} | 61.5_{4.1} | 63.0_{9.3} | 39.6_{10.3} | 70.7_{3.2} | 76.4_{8.5} | 71.4_{2.9} | 16.0  |
|  MemP | Qwen3-8B | 95.2_{2.1} | **74.4**_{6.8} | 61.7_{7.6} | 56.3_{13.4} | 76.0_{6.2} | 68.1_{8.5} | 74.3_{3.4} | 15.2  |
|  Skillos-base | Qwen3-8B | 91.4_{3.4} | 69.2_{7.7} | 56.8_{5.7} | 54.2_{13.7} | 72.0_{4.0} | 66.7_{11.0} | 70.7_{3.0} | 16.3  |
|  Skillos-gemini | Gemini-2.5-Pro | 94.3_{5.3} | 69.2_{0.0} | **77.8**_{5.7} | **75.0**_{16.5} | 80.0_{12.2} | 66.7_{2.4} | 79.3_{2.6} | 14.9  |
|  Skillos | Qwen3-8B | **95.2**_{2.9} | 71.8_{7.7} | 74.1_{13.0} | 72.9_{10.1} | **77.3**_{6.1} | **77.8**_{10.0} | **80.2**_{3.1} | **14.8**  |
...
Table 2 | Experiment results on WebShop and single-turn reasoning tasks for 3 different frozen executors. For WebShop, the averaged score, success rate (SR ↑), and the number of steps (Steps ↓) are reported. For reasoning tasks, accuracy (Acc. ↑) is reported on three datasets.

|  Methods | Curator\( \pi_S \) | WebShop |   |   | Reasoning  |   |   |   |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
|   |   |  Score | SR | Steps | AIME24 | AIME25 | GPQA | Avg. Acc  |
|  Executor \( \pi_L \): Qwen3-8B  |   |   |   |   |   |   |   |   |
|  No Memory | None | 33.3\( _{0.7} \) | 9.8\( _{0.5} \) | 20.3 | 76.0\( _{6.9} \) | 71.1\( _{10.7} \) | 61.8\( _{1.1} \) | 69.6\( _{4.7} \)  |
|  ReasoningBank | Qwen3-8B | 35.4\( _{1.1} \) | 11.4\( _{0.9} \) | 20.5 | 75.4\( _{5.0} \) | 73.2\( _{10.8} \) | 60.3\( _{3.9} \) | 69.6\( _{2.5} \)  |
|  MemP | Qwen3-8B | 35.7\( _{0.9} \) | 12.0\( _{0.5} \) | 21.3 | 75.6\( _{5.1} \) | 71.1\( _{5.1} \) | 60.6\( _{4.0} \) | 69.1\( _{4.0} \)  |
|  SKILLOS-base | Qwen3-8B | 38.6\( _{0.9} \) | 13.6\( _{0.8} \) | 20.1 | 75.6\( _{5.1} \) | 71.9\( _{6.9} \) | 59.3\( _{2.5} \) | 68.9\( _{2.5} \)  |
|  SKILLOS-gemini | Gemini-2.5-pro | 38.1\( _{1.0} \) | 13.2\( _{0.9} \) | 19.6 | 73.3\( _{1.3} \) | 71.3\( _{1.9} \) | 57.6\( _{2.8} \) | 67.4\( _{0.8} \)  |
|  SKILLOS | Qwen3-8B | 40.6\( _{0.7} \) | 16.5\( _{0.7} \) | 19.4 | 80.0\( _{3.3} \) | 76.7\( _{5.8} \) | 64.6\( _{1.3} \) | 73.8\( _{1.9} \)  |
|  Executor \( \pi_L \): Qwen3-32B  |   |   |   |   |   |   |   |   |
|  No Memory | None | 41.5\( _{0.5} \) | 12.2\( _{0.3} \) | 17.0 | 81.4\( _{1.3} \) | 72.2\( _{1.8} \) | 68.4\( _{2.0} \) | 74.0\( _{1.9} \)  |
|  ReasoningBank | Qwen3-32B | 40.4\( _{0.8} \) | 11.2\( _{1.1} \) | 17.9 | 81.1\( _{9.6} \) | 75.6\( _{5.9} \) | 66.9\( _{1.2} \) | 74.9\( _{2.2} \)  |
|  MemP | Qwen3-32B | 30.7\( _{0.7} \) | 10.1\( _{0.6} \) | 17.4 | 82.2\( _{5.1} \) | 76.7\( _{6.0} \) | 66.5\( _{2.3} \) | 75.1\( _{2.1} \)  |
|  SKILLOS-base | Qwen3-8B | 43.4\( _{0.8} \) | 12.3\( _{1.0} \) | 16.8 | 80.0\( _{3.3} \) | 75.6\( _{10.2} \) | 67.7\( _{1.5} \) | 74.7\( _{3.3} \)  |
|  SKILLOS-gemini | Gemini-2.5-pro | 45.2\( _{1.0} \) | 13.2\( _{1.1} \) | 16.6 | 77.8\( _{6.7} \) | 74.4\( _{5.9} \) | 66.2\( _{0.6} \) | 73.2\( _{1.6} \)  |
|  SKILLOS | Qwen3-8B | 49.2\( _{1.2} \) | 16.5\( _{0.6} \) | 15.9 | 85.6\( _{1.9} \) | 81.1\( _{3.3} \) | 72.4\( _{3.0} \) | 79.7\( _{1.8} \)  |
|  Executor \( \pi_L \): Gemini-2.5-pro  |   |   |   |   |   |   |   |   |
|  No Memory | None | 48.6\( _{0.3} \) | 38.4\( _{0.5} \) | 19.5 | 85.6\( _{1.9} \) | 80.0\( _{6.7} \) | 79.9\( _{1.5} \) | 81.8\( _{2.8} \)  |
|  ReasoningBank | Gemini-2.5-pro | 50.8\( _{1.5} \) | 40.2\( _{1.3} \) | 19.2 | 85.6\( _{5.1} \) | 84.4\( _{5.7} \) | 80.4\( _{2.1} \) | 83.5\( _{2.1} \)  |
|  MemP | Gemini-2.5-pro | 51.3\( _{1.2} \) | 39.8\( _{1.0} \) | 19.4 | 83.3\( _{6.9} \) | 76.7\( _{5.8} \) | 81.8\( _{3.8} \) | 80.6\( _{3.2} \)  |
|  SKILLOS-base | Qwen3-8B | 52.8\( _{1.0} \) | 39.6\( _{0.8} \) | 19.0 | 87.8\( _{3.3} \) | 83.3\( _{3.9} \) | 82.8\( _{2.7} \) | 84.6\( _{3.0} \)  |
|  SKILLOS-gemini | Gemini-2.5-pro | 54.7\( _{1.0} \) | 41.0\( _{1.2} \) | 17.8 | 90.0\( _{5.1} \) | 85.6\( _{7.7} \) | 80.7\( _{5.5} \) | 85.4\( _{5.5} \)  |
|  SKILLOS | Qwen3-8B | 56.0\( _{0.7} \) | 41.3\( _{0.8} \) | 18.3 | 92.2\( _{2.4} \) | 86.7\( _{5.5} \) | 86.8\( _{2.1} \) | 88.6\( _{1.5} \)  |
...
![img-2.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-2.jpeg)
...
atively small fraction throughout training with a slightly growing trend, showing the effectiveness of rewarding conciseness of SKILLREPO. Instead, the dominant form of adaptation is to revise and consolidate previously acquired skills.
...
![img-3.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-3.jpeg)

Figure 4 | Behaviors of the skill curator w.r.t. skill operations during training.

![img-4.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-4.jpeg)

(a) Emergence of new markdown sections

![img-5.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-5.jpeg)

(b) Emergence of meta-strategy skills
...
![img-6.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-6.jpeg)
...
Y. Yang, J. Li, Q. Pan, B. Zhan, Y. Cai, L. Du, J. Zhou, K. Chen, Q. Chen, X. Li, B. Zhang, and L. He. Autoskill: Experience-driven lifelong learning via skill self-evolution. ArXiv preprint, abs/2603.01145, 2026. URL https://arxiv.org/abs/2603.01145.
...
skill_update: If the existing skill can be improved, update the specific skill by its <skill_name>.</skill_name>
...
Parameters: {"type": "object", "properties": {"skill_name": {"type": "string", "description": "The name of the skill to delete."}, "required": {"skill_name"}} Format the arguments as a JSON object.
...
![img-7.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-7.jpeg)

Figure 12 | Prompt for using an external judge to assign a reward score  \( r^{ext} \)  for generated skill contents.

### A.4. Prompt for LLM-as-a-Judge to Obtain Correctness Signals

We present the prompts used to obtain the self-judged correctness signal  \( 1_{\xi_{i}} \)  for self-evolution via LLM-as-a-judge using the corresponding frozen agent executor as the backbone model in Figures 13, 14 for ALFWorld, reasoning, and WebShop tasks, respectively.
...
In this section, we detail the two-stage pipeline used to turn the raw training set  \( D = \{x_{i}\}_{i=1}^{N} \)  into the grouped training set  \( G = \{G_{i}\}_{i=1}^{M} \)  of Section 3.2.1. Stage 1 annotates each instance with a structured set of latent attributes via an LLM annotator (Sec. B.2.1). Stage 2 assembles groups of related tasks by retrieving, filtering, and ranking candidates under a semantic phrase-level similarity (Sec. B.2.2). For
...
System Instruction
You are an expert judge evaluating whether an embodied agent successfully completed a household task in a text-based simulator. Output a single JSON object and nothing else.
# Task
You will be given (1) the task description the agent was asked to complete, and (2) the full interaction trace between the agent and the simulator. Determine whether the agent fully completed the task.
## What "success" means
- The agent's actions must have produced the world state the task description specifies. Every condition stated in the task must hold at the end of the trace.
- If the task implies a transformation must occur before a final placement or interaction, the transformation must be evidenced in the trace before the final step.
- Credit only effects that the simulator's observations confirm. Do not credit effects that the agent merely declared, planned, or assumed. - Ignore the agent's own claims of completion; rely solely on the simulator's observation strings.
- A trace that ends with the agent stuck in a loop, exhausting its step budget, or repeatedly emitting invalid actions is a failure regardless of partial progress.
## Strictness
...
3. Not a near-duplicate:  \( \mathrm{SJ}_{\tau}(T_{s},T_{t})\leq\theta_{T} \)  and the weighted overall similarity  \( \Omega(x_{s},x_{t})\leq\sigma_{\max} \) ;
4. Not too unrelated: \(\Omega(x_s, x_t) \geq \sigma_{\min}\);
5. Progression: \( x_{t} \) introduces at least one new concept or skill, i.e. \( |C_t| > m_\tau(C_s, C_t) \) or \( |S_t| > m_\tau(S_s, S_t) \);
6. Curriculum direction:  \( d_{t} - d_{s} \geq \delta_{\min} \) .
...
You are an expert judge evaluating whether a shopping agent purchased an item that matches a user's instruction in a web-shopping simulator. Output a single JSON object and nothing else.
...
The instruction encodes a product target, zero or more required attributes of that target, and possibly a price constraint. Decompose your evaluation into the following sub-scores, then average them into a single score in [0, 1]:

1. **Product type match**: 1 if the purchased product belongs to the category named in the instruction; otherwise 0.
2. **Attribute coverage**: the fraction of attributes explicitly named in the instruction that the purchased item (with its chosen options) is shown to satisfy. If the instruction names no attributes, score 1.
...
4. **Purchase completion**: 1 if the trajectory ends with a confirmed purchase action on a concrete product page; 0 otherwise. The final 'score' is the mean of the four sub-scores. Define 'success' as 'score >= 0.5'.

## Strictness

- Award attribute credit only when the page text or the agent's selected options provide positive evidence; do not infer attributes from the absence of contradiction.

- A purchase made on the wrong product type forces score = 0 regardless of the other sub-scores.

# Output

Output exactly one JSON object and nothing else: {{ "subscores": {{ "product_type": <0|1>, "attribute_coverage": <float in [0,1]>, "price": <0|1>, "purchased": <0|1>}}, "score": <float in [0,1], the mean of subscores>, "success": <truefalse>, "rationale": "<one or="" sentences="" or="">"}\)"</one></truefalse></float></0|1></float>
...
|  Hyperparameter | ALFWorld | Value WebShop | Reasoning  |
| --- | --- | --- | --- |
|  *RL Training*  |   |   |   |
...
SkilIOS: Learning Skill Curation for Self-Evolving Agents
...
Figure 16 | System instruction used to elicit  \( Z_{i} \)  from each task in D.
...
|  \( (p_{\uparrow}, p_{\approx}, p_{\downarrow}) \) | Mode probabilities | (0.80, 0.20, 0.00)  |
|  \( [\Delta_{\min}, \Delta_{\max}] \) | Gap in EASY→HARD mode | [0.5, 3.0]  |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
|  No Memory | 85.7_{0.0} | 59.0_{8.9} | 67.9_{6.9} | 25.0_{0.2} | 38.7_{2.3} | 66.7_{0.0} | 61.2_{2.3} | 18.5  |
|  ReasoningBank | 87.6_{4.4} | 71.8_{4.4} | 63.0_{0.0} | **52.1**_{14.4} | 48.0_{10.6} | 62.5_{0.0} | 66.0_{2.7} | 17.6  |
|  MemP | 84.3_{5.1} | 57.7_{5.4} | 63.0_{0.0} | 28.1_{4.4} | 34.0_{2.8} | 62.5_{0.0} | 58.6_{1.0} | 19.3  |
|  SKILLOS-base | 86.7_{1.6} | 61.5_{0.0} | 66.7_{0.0} | 41.7_{6.2} | 38.7_{16.0} | 68.1_{2.4} | 63.6_{3.9} | 17.7  |
|  SKILLOS-gemini | **96.2**_{1.6} | 61.5_{13.3} | 74.1_{3.7} | 31.2_{12.5} | 66.7_{4.6} | 68.1_{2.4} | 71.2_{2.9} | 16.1  |
|  SKILLOS | 88.6_{0.0} | **84.6**_{13.3} | **77.8**_{4.0} | 37.5_{17.2} | **68.0**_{8.0} | **68.1**_{2.4} | **73.1**_{2.7} | **15.5**  |
...
![img-8.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-8.jpeg)

#### Workflow

relate to existing skills, showcasing composition capabilities

1. Exhaustive Search: First, confirm a thorough search for the primary target object. Use a systematic method, such as the Final Search by Prioritizing Likely Variations, which to check all probable locations.
...
- Dicheware: If a Plate is missing, use a Ball.
- Utensils: If a Pick is missing, use a Spot or Buttermits.
- Cookware: If a Pot is missing, use a Pot.
- Cleaning: If a Yeast is missing, use a Cloth.
...
name: Relate Incenter-Vertex Distance to Triangle Properties

description: Calculate triangle properties like angles or side lengths using the distance from a vertex to the Incenter, the Irredu, and the semiperimeter.

#### Workflow

When you know or can calculate the distance from a vertex (e.g., A) to the Incenter (I), you can use the following two formulae to find other triangle properties.

#### Method 1: Find the Angle at the Vertex

Skill that shows branching out reasoning behavior

1. Formula: The distance (a) is related to the Irredu (r) and the angle (k) by: (k = r / 1) or (k/2)

2. Application: If (a) and (r) are known, you can solve for  \( \{a\}/\{b\}/\{c\} \) . From  \( \{a\}/\{b\}/\{c\} \) , you can find  \( \{a\}/\{b\}/\{c\} \)  (using  \( \{a\}/\{b\} \)  +  \( \{a\}/\{b\} \)  + 1) and subsequently  \( \{a\}/\{b\} \)  and  \( \{a\}/\{b\} \)  using double angle formulae.

#### Method 2: Relate to Side Lengths

1. Formula: The distance (a) is related to the Irredu (r), the semiperimeter (r), and the side (k) (opposite to vertex A) by: (k' = r' + 1) - a1'

2. Application: If (b) and (r) are known, you can solve for  \( \{b\} \)  (r, k). This provides a direct equation linking the semiperimeter and a side length.

#### Prerequisite Constraints

• The Irredu (r) must be known.
• The distance from a vertex to the Incenter (e.g., (k)) must be known or calculable.

(b) Skill curated by SkillOS for reasoning tasks
...
description: Calculate the number of ways to fill a 3x9 grid with numbers 1-9 such that each row is a permutation and each 3x3 block contains all numbers 1-9.
...
1. Define Constraints:  \( R_{B} \geq 1 \)  //  \( R_{A} \)
2. Categorize Elements (group elements by which complements they belong to).
3. Set up linear system:
- Size constraint:  \( \Sigma_{\Sigma} \)  =  \( \Sigma_{\Sigma} \)  =  \( \Sigma_{B} \)
- Exhaustion constraint:  \( \Sigma_{\Sigma} \)  =  \( \Sigma_{\Sigma} \)  =  \( \Sigma_{\Sigma} \)
...
##### Example: Sudoku-like 3x3 Block Counting

Problem: Read partition  \( R_{A} \)  (1,2,3),  \( R_{A} \)  (3,4,5),  \( R_{A} \)  (5,6,7), and  \( R_{B} \)  (7,8,9). Find # of new partitions  \( R_{B} \)  such that  \( R_{B} \cap R_{A} \geq \emptyset \) .

- Categories: C1=(4,5,6) C2=(7,8,9) C3=(1,2,3) (each lies in two complements)

- Equations: n_2n_n_2n_3, n_2n_n_2n_3, n_2n_n_2n_3, n_2n_n_2n_3 (for C1), ...

- Integer solutions: 4 families

Case 1: R_B1=C3, R_B2=C3, R_B3=C1 ways = 1 + (30° × 2H)

Case 2: mixed, ways = 81 + (35) ...

- Sum errors at 4 cases: + 10704; + 10,096
...
![img-9.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-9.jpeg)

Start in the middle of the room

![img-10.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-10.jpeg)

Use desklamp1

![img-11.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-11.jpeg)

(15+ wasted moves: searching)

![img-12.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-12.jpeg)

Go to drawer3

Skillos

![img-13.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-13.jpeg)

Start in the middle of the room

![img-14.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-14.jpeg)

Take CD1

![img-15.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-15.jpeg)

Use desklamp1

![img-16.jpeg](file:///data/user/0/host.exp.exponent/files/ExperienceData/%2540anonymous%252Fanthology-74baf6d1-d49f-4fc8-98f3-ca70208bde8b/assets/mrgif5ih033kh2k0-img-16.jpeg)
